      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading && <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-500">Loading service items...</td></tr>}
              {!loading && filteredServices.length === 0 && <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">No service items found</td></tr>}
              {filteredServices.map((s) => (
                <tr key={s.id} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-medium text-gray-800">{s.name}</td>
                  <td className="px-6 py-4 text-gray-600">{s.serviceName}</td>
                  <td className="px-6 py-4 font-semibold text-green-600">${s.possiblePrice}</td>
        ...
        <div className="block md:hidden divide-y divide-gray-100">
          ...
          {!loading && filteredServices.length === 0 && (
            <div className="p-6 text-center text-gray-400 text-sm">No service items found</div>
          )}
          {!loading && filteredServices.map((s) => (
            <div key={s.id} className="px-4 py-3 hover:bg-gray-50">
              <div className="flex items-center justify-between gap-2 mb-1">
                ...
                <span className="text-[13px] font-bold text-green-600">${s.possiblePrice}</span>
              </div>
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="text-[12px] text-gray-400">{s.serviceName}</span>
