...
import AgencyFormModal, { AgencyFormData } from "./AgencyFormModal";
...
export default function AgencyTable() {
  ...
  const loadAgencies = useCallback(async (page: number, searchQuery: string) => {
    setLoading(true);
    try {
      const res = await SetupService.getAgencies(page, itemsPerPage);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      loadAgencies(currentPage, search);
    }, 400);
    return () => clearTimeout(timer);
  }, [currentPage, search, loadAgencies]);
  ...
  const handleFormSubmit = async (data: AgencyFormData) => {
    try {
      if (mode === "add") {
        await SetupService.createAgency(data);
        toast.success("Agency created successfully");
      } else {
        if (!selectedAgency) return;
        await SetupService.updateAgency(selectedAgency.id, data);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await SetupService.deleteAgency(selectedAgency.id);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-[13px] text-[#495057] font-medium hidden sm:block">
            ...
            <span className="text-gray-400">List Agencies</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm overflow-hidden">
          ...
          <div className="p-3 sm:p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              {canAdd && (
                <button
                  onClick={() => { setMode("add"); setSelectedAgency(null); setOpenModal(true); }}
                ...
                >
                  <span className="text-lg">+</span> Add Agency
                </button>
              )}
            </div>
                ...
                {loading && agencies.length === 0 ? (
                  ...
                  agencies.map((u) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-[#878a99]">{u.country}</td>
                      <td className="p-3">{u.phone || "N/A"}</td>
                      <td className="p-3 font-medium text-center">{u.address}</td>
                      <td className="p-3 text-center">
                      ...
                      <td className="p-3 font-medium text-center">{u.userName}</td>
                      <td className="p-3">
                        <div className="flex items-center justify-center gap-2">
                          {canEdit && (
                            <button
                              onClick={() => { setMode("edit"); setSelectedAgency(u); setOpenModal(true); }}
                          ...
                          {canDelete && (
                            <button
                              onClick={() => { setSelectedAgency(u); setOpenDelete(true); }}
